...
const UsuarioNesting = {
  ...
  chats: async (parent) => {
    const chats = await Chat.find({ _id: { $in: parent.chats } }).populate(
      "mensajes"
    );
    ...
    for (const chat of chats) {
      for (const msg of chat.mensajes.reverse()) {
        ...
        if (
          msg.usuario.toString() !== parent.id &&
          !msg.recibido.some((usID) => usID.toString() === parent.id)
        ) {
  ...
  carrera: async (parent) => {
    console.log("parent.carrera: ", parent.carrera.toString());
    const carrera = await Carrera.findById(parent.carrera.toString());
    return carrera;
  }
};
