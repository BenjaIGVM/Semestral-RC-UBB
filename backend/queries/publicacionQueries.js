...
const publicacionQueries = {
  ...
  buscarPublicacionGrupo: async (root, args) => {
    const publicacion = await Publicacion.find({ enGrupo: args.grupo }).sort({
      fecha: -1,
    });
  ...
  feedRecomendations: async (root, args) => {
    ...
    const filteredPosts = await Publicacion.aggregate([
      ...
      },
      {
        $project: {
          popularityIndex: 0,
          createdAtDay: 0 /*,  matchingInterests: 0, maxInterestValue: 0 */,
        },
      },
      { $limit: 20 },
    ]).catch((error) => {
    ...
    if (filteredPosts.length < 10) {
              ...
              ...new Set(
                await Promise.all(
                  user.intereses
                    .slice(0, 20)
                    .map(
                      async (info) => (await Tag.findById(info.tag)).categoria
                    )
                )
              ),
            ].map(async (cat) => await Category.findById(cat))
          )
        ).map((cat) => {
          toSearch.push(...cat.tags);
        });
      }
      ...
      if (nFiltered.length < 10) {
        const finalFilter = await Publicacion.aggregate([
          ...
          {
            $match: {
              _id: { $nin: nFiltered.map((post) => post._id) },
              esComentario: { $exists: false },
            },
          },
          {
